...
// Le panneau se pose à bottom: NAV_HEIGHT, juste au-dessus de la barre de
// navigation. On prend une valeur un peu INFÉRIEURE à la hauteur réelle de la
// barre (~75 px) pour que le panneau la chevauche de quelques pixels (masqués,
// la barre est au-dessus) : ainsi aucun interstice ne laisse voir la carte.
...
export default function RouteSheet({
  ...
  return (
    <section
      ...
      // Posé juste au-dessus de la barre de navigation (bottom: NAV_HEIGHT),
      // pas au ras du bas : le bouton d'action reste visible.
      style={{ height, bottom: NAV_HEIGHT }}
      className="pointer-events-auto fixed inset-x-0 z-[1001] flex flex-col rounded-t-[2rem] bg-white shadow-[0_-16px_40px_-20px_rgba(15,23,42,0.4)] transition-[height] duration-300 ease-out"
    >
      ...
      <div
        // min-h-0 : indispensable pour qu'un conteneur flex-1 puisse se
        // rétrécir et défiler au lieu de pousser le pied hors du panneau.
      ...
      >
        ...
        {expanded && isResult && (
          <div className="mt-4 flex flex-col gap-4">
            ...
            {/* Perturbations : uniquement en mode Transports. Pour un trajet
                transit, elles portent sur l'itinéraire réel (lignes connues). */}
            {profile === 'transit' &&
      ...
      <div className="shrink-0 rounded-b-[2rem] border-t border-slate-100 bg-white px-6 pb-4 pt-3">
        {isResult ? (
          <button
            type="button"
            onClick={onStart}
