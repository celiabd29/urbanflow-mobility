...
import { countPendingReports, queueReport } from '@/lib/offlineStore'
import { notifyPendingChanged, onPendingChanged } from '@/lib/reportSync'
...
export default function ReportIncident() {
  ...
  // false | 'sent' (envoyé) | 'queued' (mis en file hors ligne)
  const [done, setDone] = useState(false)
  // Nombre de signalements en attente d'envoi (créés hors ligne).
  const [pending, setPending] = useState(0)

  // Compteur de la file, rafraîchi quand un signalement est mis en file ou envoyé.
  useEffect(() => {
    let active = true
    const refresh = () =>
      countPendingReports().then((n) => active && setPending(n))
    refresh()
    const unsubscribe = onPendingChanged(refresh)
    return () => {
      active = false
      unsubscribe()
    }
  ...
  // Met le signalement en file locale (hors ligne ou panne réseau) : pas une
  // erreur, un statut « en attente ». Il partira au retour de connexion.
  ...
  async function handleSubmit(event) {
    ...
    // Hors ligne : on met directement en file, sans tenter le réseau.
    if (!navigator.onLine) {
    ...
    try {
      ...
      // Pas de réponse serveur = panne réseau -> file d'attente, pas d'erreur.
      if (!err.response) {
        await enqueue(payload)
      } else {
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden bg-[#f8fafc]">
      ...
      <div className="absolute inset-x-0 top-0 z-[1000] flex items-center gap-3 px-5 pt-6">
        ...
        {/* Badge : signalements créés hors ligne, en attente d'envoi. */}
        {pending > 0 && (
          <span className="flex h-11 shrink-0 items-center gap-1.5 rounded-full bg-amber-100 px-3 text-sm font-medium text-amber-700 shadow-md">
