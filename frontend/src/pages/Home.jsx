...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Trajets récents en network-first : frais si possible, sinon le dernier
    // instantané IndexedDB pour rester consultables hors ligne.
    networkFirst('trips', () =>
      api.get('/carbon/historique/').then((r) => r.data),
    )
      .then(({ data, fromCache }) => {
        if (!active) return
        setTrajets(data.trajets.slice(0, RECENT_LIMIT))
        setTripsFromCache(fromCache)
      })
      .catch(() => {})
  ...
  // Supprime un trajet depuis l'accueil : retire de la liste affichée et met à
  // jour le cache complet (partagé avec l'écran /trajets et le hors ligne).
  async function handleDelete(id) {
    await deleteTrajet(id)
    setTrajets((prev) => prev.filter((t) => t.id !== id))
    const cached = await readCache('trips')
    if (cached?.trajets) {
      saveCache('trips', {
        ...cached,
        trajets: cached.trajets.filter((t) => t.id !== id),
        count: Math.max(0, (cached.count || 1) - 1),
      })
    }
  }
