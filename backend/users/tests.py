...
class LoginThrottleTests(APITestCase):
    """Le login doit bloquer le brute-force au-delà du quota (429)."""

    def setUp(self):
        # Repart d'un compteur de throttle vierge.
        cache.clear()
        self.url = reverse("users:login")

    def test_login_is_rate_limited_after_quota(self):
        payload = {"email": "inconnu@urbanflow.app", "password": "wrong-pass"}
        # Le quota réel est neutralisé pendant la suite (settings.TESTING) : on
        # force ici un rythme bas directement sur le throttle, indépendamment du
        # rechargement des settings. 3 tentatives autorisées, la 4e est bloquée.
        with mock.patch.dict(
            ...
            # La 4e dépasse le quota -> 429 Too Many Requests.
            response = self.client.post(self.url, payload, format="json")
            self.assertEqual(
                response.status_code, status.HTTP_429_TOO_MANY_REQUESTS
            )
