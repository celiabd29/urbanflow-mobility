...
@api_view(["DELETE"])
@permission_classes([IsAuthenticated])
def delete_trajet_view(request, pk):
    """
    DELETE /api/carbon/trajets/<pk>/

    Supprime un trajet de l'utilisateur connecté. Le filtre sur `user` garantit
    qu'on ne peut effacer que ses propres trajets, jamais ceux d'un autre.
    """
    deleted, _ = Trajet.objects.filter(user=request.user, pk=pk).delete()
    if not deleted:
